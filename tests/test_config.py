...
def test_config_get_schema():
    exit_code, _, _, jsonout = run_cli(["config", "get-schema", "env"])
    assert exit_code == 0
    assert jsonout["default"] == "prod"

    exit_code, _, _, jsonout = run_cli(["config", "get-schema", "defaultOutput"])
    assert exit_code == 0
    assert "default" not in jsonout


def test_config_set():
    exit_code, _, _, _ = run_cli(["config", "set", "workers", "999"])
    assert exit_code == 0


def test_config_get():
    exit_code, _, _, jsonout = run_cli(["config", "get", "workers"])
    assert exit_code == 0
    assert jsonout == 999


def test_config_delete():
    exit_code, _, _, jsonout = run_cli(["config", "delete", "workers"])
    assert exit_code == 0
